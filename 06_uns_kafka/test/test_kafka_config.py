...
is_configs_provided: bool = settings.get(
    "kafka.config") is not None and "bootstrap.servers" in settings.get("kafka.config")
...
def test_mqtt_config():
    ...
    assert mqtt_transport in (
        None, "tcp", "websockets"), f"Invalid value for key 'mqtt.transport':{mqtt_transport}"
    ...
    assert mqtt_qos in (
        None, 0, 1, 2), f"Invalid value for key 'mqtt.qos':{mqtt_qos}"
    ...
    assert clean_session in (
        True, False, None), f"Invalid value for key 'mqtt.clean_session'{clean_session}"
    ...
    assert (username is None and password is None) or (
        isinstance(username, str) and len(username) > 0 and isinstance(
            password, str) and len(password) > 0
    ...
    assert (keep_alive is None) or (keep_alive >
    ...
    timestamp_attribute: str | None = settings.get(
